...
Adaptación del original que en su página web (http://www.averdevelopment.com/python/html) indica :
    ...
    for editBox in editBoxes:
        ...
        if user32.SendMessageA(editBox, EM_GETLINECOUNT, 0, 0) < 2:
            ...
            if newHeight != oldHeight:
                ...
                w = width(editBoxRect)
                user32.MoveWindow(editBox, topLeft.x, topLeft.y, w, newHeight, False)
...
def Message(msg, id=260, ok=None, titulo=None):
    ...
    def DlgProc(hwnd, uMsg, wParam, lParam):
        if uMsg == WM_INITDIALOG:
            if titulo :
                user32.SetWindowTextA(hwnd, titulo)
...
def AskString(prompt, default = '', id=261, ok=None, cancel=None, titulo=None):
...
def AskYesNoCancel(question, default=0, yes=None, no=None, cancel=None, id=262, titulo=None):
    ...
    def __init__(self, title="Working...", maxval=0, label="", id=263):
        ...
        def DlgProc(hwnd, uMsg, wParam, lParam):
            if uMsg == WM_INITDIALOG:
                ...
                user32.SetWindowTextA(user32.GetDlgItem(hwnd, IDCANCEL), "Cancelar")
            if uMsg == WM_COMMAND and LOWORD(wParam) == IDOK:
                user32.EndDialog(hwnd, IDOK)
                return True
            if uMsg == WM_COMMAND and LOWORD(wParam) == IDCANCEL:
...
def AskFileForOpen(
    ...
    def hookProc(hdlg, uiMsg, wParam, lParam):
        if uiMsg == WM_INITDIALOG:
            ...
            if titulo :
                user32.SetWindowTextA(dialog, titulo)
            if message:
...
#-------------------------------------------------------------------------------------------------------------------------
def mensaje( texto, aceptar = "Aceptar", titulo = "Mensaje" ) :
    Message( texto, ok = aceptar, titulo = titulo )
    
#-------------------------------------------------------------------------------------------------------------------------
def pregunta( texto, titulo = "Pregunta", default = 0 ) :
    resp = AskYesNoCancel( texto, yes = "Sí", no = "No", cancel ="Cancelar", titulo = titulo, default = default )
    return True if resp == 1 else ( False if resp == 0 else None )
    
#-------------------------------------------------------------------------------------------------------------------------
class BarraProgreso :
    def __init__( self, titulo ) :
        self.pb = ProgressBar( titulo )
        self.total = 0
        self.actual = 0
    def ponEtiqueta( self, eti ) :
        self.pb.label( eti )
...
#-------------------------------------------------------------------------------------------------------------------------
def buscaFichero( titulo, fichero="", dirInicial="", listaExtensiones=[('Todos los ficheros (*.*)', '*.*')], mensaje = None ) :
    """
    """
    if fichero :
        if dirInicial :
            ...
            dirInicial, fichero = os.path.split(fichero)
        
    return AskFileForOpen( message=mensaje, titulo=titulo, typeList=listaExtensiones, defaultLocation=dirInicial, actionButtonLabel="Elegir", cancelButtonLabel="Cancelar", defaultfn=fichero )
    
#-------------------------------------------------------------------------------------------------------------------------
def buscaCarpeta( titulo, dirInicial ) :
    return AskFolder(titulo, defaultLocation=dirInicial, actionButtonLabel="Elegir", cancelButtonLabel="Cancelar" )
    

#-------------------------------------------------------------------------------------------------------------------------
def leeTexto( titulo, mensaje, texto ):
    return AskString(mensaje, default = texto, ok="Aceptar", cancel="Cancelar", titulo=titulo)

#-------------------------------------------------------------------------------------------------------------------------

#~ if __name__ == '__main__':
    #~ mensaje( "hola", "que", aceptar = "SEFADFADFADFADseguir" )
    #~ pregunta( "hjashjash!", "titulo" )
    
    #~ bp = BarraProgreso( "titulo" )
    #~ bp.ponEtiqueta( "Etiqueta" )
    #~ bp.ponTotal( 100 )
    ...
    #~ print buscaFichero( "titulo", "..", [('All Files (*.*)', '*.*'), ('C Files (*.c, *.h)', '*.c;*.h')] )
    
    #~ print buscaCarpeta( "titulo", "c:/mgd" )
#~ print buscaFichero( "titulo", "../p.py", listaExtensiones=[('All Files (*.*)', '*.*'), ('C Files (*.c, *.h)','*.c;*.h')], mensaje="Hola" )
    
#~ print leeTexto( "titulo", "Hola", "jiojo" )
    
#~ mensaje( "el fichero\n%s\nno existe"%"c:leddl dlkdl fldkfldf ldf dlkfldfk ldkfldkfldk wrkejwr wekrjwelkrj lwekjr lwekrj lewkjrlwekjrlwejrlew klewjrlewkr" )
