...
############################################################################################################
def juntar( cFich ) :
    resp = g.AskYesNoCancel( "Unimos el fichero creado al de PROVEEDORES si se ha generado anteriormente, y creamos uno conjunto", yes="Si", no="No", cancel="" )
    if resp != 1 :
        return

    fichCli = g.AskFileForOpen( "Indica el fichero generado anteriormente de PROVEEDORES", defaultLocation= os.path.split(cFich)[0] )
